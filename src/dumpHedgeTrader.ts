  ...
  async processSnapshot(snapshot: MarketSnapshot): Promise<void> {
    ...
    if (phase.kind === "WaitingForHedge") {
      ...
      if (timeElapsedMinutes >= this.stopLossMaxWaitMinutes) {
        if (oppositeTokenId) {
          const maxStopLossSum = 1 + this.stopLossPercentage;
          const currentSum = phase.leg1EntryPrice + oppositeAsk;
          if (currentSum > maxStopLossSum) {
            logPrintln(
              `${marketName}: STOP LOSS WAITING - Current sum $${currentSum.toFixed(4)} exceeds cap $${maxStopLossSum.toFixed(4)}`
            );
            return;
          }
          logPrintln(
