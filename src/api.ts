...
export class PolymarketApi {
  ...
  private async fetchJson<T>(url: string, errorPrefix: string): Promise<T> {
    const controller = new AbortController();
    const timeout = setTimeout(() => controller.abort(), 10000);
    try {
      const res = await fetch(url, { signal: controller.signal });
      if (!res.ok) {
        throw new Error(`${errorPrefix} (status: ${res.status})`);
      }
  ...
  async getMarketBySlug(slug: string): Promise<Market> {
    ...
    const json = await this.fetchJson<{ markets?: Record<string, unknown>[] }>(
      url,
      `Failed to fetch market by slug: ${slug}`
    );
  ...
  async getMarket(conditionId: string): Promise<MarketDetails> {
    ...
    const json = await this.fetchJson<MarketDetails>(url, "Failed to fetch market");
