...
import { ROUND_DURATION_SECONDS, isSupportedAsset } from "./constants";
import { sleep } from "./utils";

function parseArgs(): { simulation: boolean } {
  const args = process.argv.slice(2);
  const production = args.includes("--production");
  const simulation = production ? false : args.includes("--simulation");
  return { simulation };
}
...
async function discoverMarketForAsset(
  ...
  if (!isSupportedAsset(assetLower)) {
    throw new Error(
      `Unsupported asset: ${asset}. Supported: BTC, ETH, SOL, XRP`
    );
  }
  const slugPrefix = assetLower;

  const periodDurationSecs = ROUND_DURATION_SECONDS;
  ...
  for (const { asset, marketName, market } of validMarkets) {
    ...
    (async () => {
      ...
      for (;;) {
        ...
        const nextPeriodTimestamp = currentMarketTimestamp + ROUND_DURATION_SECONDS;
        const currentTime = Math.floor(Date.now() / 1000);
        const sleepSecs =
        ...
        const currentPeriod = Math.floor(now / ROUND_DURATION_SECONDS) * ROUND_DURATION_SECONDS;
